...
const ErrorList = ({ errors = [], isLoading }: ErrorListProps) => {
  if (isLoading) return <p data-testid="errors-loading">Loading...</p>;

  const Errors = errors.map(
    (row: ProcessErrorLogResultModel, index: number) => (
      <ErrorTile key={index} row={row} />
    )
  );
  return (
    <div data-testid="errorlist">
      {Errors.length ? (
        Errors
      ) : (
        <Typography data-testid="no-errors" variant="h5">
...
const ErrorTile = ({ row }: { row: ProcessErrorLogResultModel }) => {
  const classes = useStyles();
  if (!row) return null;
